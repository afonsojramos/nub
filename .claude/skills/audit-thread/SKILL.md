...
- **2026-07-12 (sandbox delta audit — the STALE-PIN scar, twice):** the audit started from a local `sandbox-primitives` worktree stranded at an old SHA (`5f99fcfbb2`) while `origin/sandbox-primitives` was `4719fdf9ce` — a stale local checkout has now mis-pinned TWO consecutive sandbox audits. The agent self-corrected by fetching origin, but a stale pin auditing the WRONG (pre-merge) code is the same wrong-target garbage as a wrong-major reference. → **Gate 1** (for a repo-branch target: `git fetch origin` + pin `origin/<branch>@<sha>` + verify SHA-reachability before reading; never trust a local worktree/branch checkout to be current).
- **2026-07-09 (sandbox security audit — the false-NEGATIVE axis):** a security-audit finding "this hole is IRREDUCIBLE / can't be closed" is ITSELF a claim to exhaust — audits produce false NEGATIVES on fixability, not just false positives. Two "irreducible" verdicts collapsed under scrutiny: a macOS `KERN_PROCARGS2` env-read declared "Seatbelt cannot block it, exhaustively proven" (the agent tested an EXACT `(sysctl-name "kern.procargs2")` deny, which silently never matches because the kernel appends the pid → `kern.procargs2.<pid>`; a `sysctl-name-PREFIX` deny closes it), and a Windows "must live under a nub-owned C:\ store" limitation traced to a code comment claiming "proven by the CI probe" where NO probe ran. **RULE: before surfacing any "irreducible"/"can't-close" verdict, EXHAUST it** — fresh-eyes brainstorm sub-agents (untainted by the prior "can't"), web research (how do Chromium/Firefox/Codex/SRT solve it), probe the FULL API surface + docs of the mechanism (every Landlock ABI, the complete Seatbelt operation vocabulary, seccomp, EndpointSecurity, entitlements, kernel source), enumerate EVERY vector × EVERY candidate closure, empirically test each. "Irreducible" ships only WITH the exhaustion record. Trying one thing, hitting a wall, and giving up is the banned pattern — we only need ONE way that works.
