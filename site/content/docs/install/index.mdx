...
The OS-level script sandbox — a network-blocked, filesystem-scoped sandbox around every install/lifecycle script — is compiled in but **off by default**. Opt in with the neutral `paranoid` / `npm_config_paranoid` setting, which also flips the advisory gate to fail-closed. The sandbox covers macOS and Linux; Windows is a passthrough.
