...
Like pnpm, Nub keeps package files in a global content-addressed store and links them into `node_modules` with reflinks or hardlinks when the filesystem supports them. The benchmark below is the warm path for a Vite (React) app: packages are already on disk, `node_modules` is removed, and the installer rebuilds the project layout.

<Bench
  label="warm frozen install · Vite (React) · 473 deps · macOS"
