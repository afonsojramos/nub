# Shared helpers for the nub Windows sandbox validation probes.
...
# Controlled root we fully own. Grant BOTH AppContainer well-known groups RX (inherited):
...
function New-ControlledDir([string]$tag) {
    Initialize-ProbeRoot
    $d = Join-Path $script:ProbeRoot ("$tag-" + [guid]::NewGuid().ToString('N'))
    New-Item -ItemType Directory -Path $d -Force | Out-Null
    $d
}

# Grant a specific AppContainer SID ReadAndExecute (inherited) on a path.
function Grant-AcRx([string]$path, [System.Security.Principal.SecurityIdentifier]$acSid) {
    $acl = Get-Acl $path
    $acl.AddAccessRule((New-Object System.Security.AccessControl.FileSystemAccessRule($acSid,'ReadAndExecute','ContainerInherit,ObjectInherit','None','Allow')))
    Set-Acl -Path $path -AclObject $acl
}
function Grant-AcModify([string]$path, [System.Security.Principal.SecurityIdentifier]$acSid) {
    $acl = Get-Acl $path
    $acl.AddAccessRule((New-Object System.Security.AccessControl.FileSystemAccessRule($acSid,'Modify','ContainerInherit,ObjectInherit','None','Allow')))
    Set-Acl -Path $path -AclObject $acl
}

# Compile the tiny native child once, under the controlled root so AppContainer SIDs have RX.
function Build-ProbeChild {
    Initialize-ProbeRoot
    $exe = Join-Path $script:ProbeRoot 'probe-child.exe'
