...
#          `nub pm use nub` (zero pnpm-named files, nub.lock present, a
...
# nub.lock (the engine must resolve+link from the renamed lockfile and the
...
leg_nub() {
  ...
  [ -f "$proj/nub.lock" ] || { echo "FAILED: use nub left no nub.lock" >>"$log"; return 1; }
  local pnpm_named
  ...
  nub_step "$log" "$proj" pnpm install --frozen-lockfile \
    || { echo "FAILED: nub-mode frozen install from nub.lock (exit $?)" >>"$log"; return 1; }
  fixture_post_check "$fixture" "$proj" "$log" || return 1
  ...
  [ -f "$proj/pnpm-lock.yaml" ] && [ ! -f "$proj/nub.lock" ] \
    || { echo "FAILED: use pnpm did not rename nub.lock back" >>"$log"; return 1; }
  wipe_node_modules "$proj"
