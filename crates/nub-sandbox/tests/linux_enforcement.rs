...
//!
//! Each test compiles a surface policy, applies it, SPAWNS the child under Landlock
//! + seccomp, and asserts the kernel allowed or denied the action. Every confinement
//! assertion is paired with a NEGATIVE CONTROL (the axis lifted → the same action
//! succeeds) so a pass can't be hollow. Linux-only, and a no-op (skips) on a kernel
//! without Landlock — CI runs it on `ubuntu-24.04` (kernel 6.x, Landlock present);
//! the dev host proves it in a Lima/Colima Ubuntu 24.04 VM.
//!
...
fn landlock_available() -> bool {
    ...
    let abi = unsafe { libc::syscall(SYS_LANDLOCK_CREATE_RULESET, std::ptr::null::<libc::c_void>(), 0usize, 1u64) };
    abi >= 2
}
...
impl Fixture {
    ...
    fn ctx(&self, env: &[(&str, &str)]) -> CompileCtx {
        CompileCtx {
            ...
            cwd: self.proj.clone(),
            trusted: true,
            ambient_env: env.iter().map(|(k, v)| (k.to_string(), v.to_string())).collect(),
    ...
    fn run(&self, surface: Value, env: &[(&str, &str)], program: &str, args: &[&str]) -> (i32, String) {
        let policy = compile(&surface, &self.ctx(env)).expect("compiles");
        let spec = CommandSpec::new(program).args(args.iter().copied()).cwd(&self.proj);
...
fn read_confine_allows_project_denies_outside() {
    ...
    assert!(f.ok(confine.clone(), CAT, &[&s(&f.proj.join("pub.txt"))]), "project read");
    assert!(f.ok(confine.clone(), CAT, &[&s(&f.proj.join("sub/nested.txt"))]), "nested read");
    assert!(!f.ok(confine.clone(), CAT, &[&s(&f.home.join(".ssh/id_rsa"))]), "home secret denied");
    // negative control — relaxed fs reads the secret fine:
    assert!(
        f.ok(serde_json::json!({ "fs": true }), CAT, &[&s(&f.home.join(".ssh/id_rsa"))]),
...
fn generous_read_denies_dotenv() {
    ...
    assert!(f.ok(surface.clone(), CAT, &[&s(&f.proj.join("pub.txt"))]), "pub readable");
    assert!(!f.ok(surface.clone(), CAT, &[&s(&f.proj.join(".env"))]), ".env denied");
    assert!(!f.ok(surface, CAT, &[&s(&f.proj.join("sub/.env"))]), "nested .env denied");
    assert!(
        f.ok(serde_json::json!({ "fs": true }), CAT, &[&s(&f.proj.join(".env"))]),
...
fn write_confine_allows_target_denies_rest() {
    ...
    assert!(f.ok(wc.clone(), TOUCH, &[&s(&f.proj.join("writable/ok.txt"))]), "write inside grant");
    assert!(!f.ok(wc.clone(), TOUCH, &[&s(&f.proj.join("blocked.txt"))]), "write project root denied");
    assert!(!f.ok(wc, TOUCH, &[&s(&f.home.join("w.txt"))]), "write outside denied");
    assert!(
        f.ok(serde_json::json!({ "fs": true }), TOUCH, &[&s(&f.home.join("w2.txt"))]),
...
fn confine_not_dodgeable_via_symlink_or_dotdot() {
    ...
    let confine = serde_json::json!({ "fs": ["./"] });
    assert!(!f.ok(confine.clone(), CAT, &[&s(&link)]), "symlink escaping read-confine denied");
...
fn ancestor_proc_environ_is_unreadable() {
    ...
    let f = fixture();
    // The ancestor is THIS test process; its REAL environ holds `PATH=` (the secret
    // store). A scrubbed child (env:false → empty own env) tries to read the
    // ancestor's `/proc/<ppid>/environ` — the ONLY route to that env, since its own
    // is empty. `$PPID` is the shell's parent = the test process (which spawns sh
    // directly). Under read-confine (/proc never granted) the read is denied.
    let read_environ = "/bin/cat /proc/$PPID/environ 2>/dev/null || true";
    let (_c, out) = f.run(
        serde_json::json!({ "fs": ["./"], "env": false }),
        &[],
        SH,
        &["-c", read_environ],
    );
    ...
    // negative control — fs relaxed (still env-scrubbed child) → the ancestor environ
    // IS readable, proving the block above is the fs read-confine, not env-scrub.
    let (_c2, out2) = f.run(
        serde_json::json!({ "fs": true, "env": false }),
        &[],
        SH,
        &["-c", read_environ],
    );
...
fn env_scrub_strips_secret_keeps_baseline() {
    ...
    assert!(f.run(strip.clone(), env, SH, &["-c", "test -n \"$PATH\""]).0 == 0, "PATH kept");
    assert!(f.run(strip, env, SH, &["-c", "test -z \"$MY_SECRET_TOKEN\""]).0 == 0, "secret stripped");
    assert!(
        f.run(serde_json::json!({ "env": true }), env, SH, &["-c", "test -n \"$MY_SECRET_TOKEN\""]).0 == 0,
...
fn seccomp_denies_net_ptrace_and_vmread() {
    ...
    assert_eq!(f.run(net_deny.clone(), &[], &probe, &["socket"]).0, 42, "AF_INET socket denied");
    // ptrace + process_vm_readv denied whenever sandboxing (here via net enforce).
    assert_eq!(f.run(net_deny.clone(), &[], &probe, &["ptrace"]).0, 42, "ptrace denied");
    assert_eq!(f.run(net_deny, &[], &probe, &["vmread"]).0, 42, "process_vm_readv denied");

    // negative control — no sandbox (fs:true, no other axis) → every syscall goes through.
    let relaxed = serde_json::json!({ "fs": true });
    assert_eq!(f.run(relaxed.clone(), &[], &probe, &["socket"]).0, 0, "neg: socket allowed unsandboxed");
    assert_eq!(f.run(relaxed.clone(), &[], &probe, &["ptrace"]).0, 0, "neg: ptrace allowed unsandboxed");
    assert_eq!(f.run(relaxed, &[], &probe, &["vmread"]).0, 0, "neg: vmread allowed unsandboxed");
}
...
fn enforcement_is_full_on_a_landlock_kernel() {
    ...
    assert!(prepared.degradation.is_full(), "read-confine fully enforces on a Landlock kernel");
}
